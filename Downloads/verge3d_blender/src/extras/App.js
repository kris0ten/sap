...
class App extends EventDispatcher {
    ...
    constructor(container, ctxSettings, preloader) {
        ...
        if (Detector.isSafariWithMultisamplingBug()) {
            console.warn('v3d.App: disabling buggy context AA on Safari 15.4/15.5');
    ...
    enableControls(element) {
        ...
        if (this.controls) {
            camera.controls.assignToControls(this.controls);
    ...
    enablePostprocessing(effects) {
        ...
        if (!this.postprocessing) {
            ...
            if (Detector.isSafariWithMultisamplingBug()) {
                console.warn('v3d.App: disabling buggy multisampling on Safari 15.4/15.5');
                aaSamples = 0;
            }
        ...
        for (let i = 0; i < effects.length; i++) {
            ...
            case 'ao': // 'ao' key is deprecated
                ...
                if (!gtaoGenerator) {
                    ...
                    /**
                     * Initialize gtaoMap on the renderer beforehand to make
                     * shader precompilation use correct GTAO settings and not
                     * compiling them for nothing.
                     */
